...
-opaque glint(MTS) :: {glint, config(), command_node(MTS)}.

-type args_count() :: {eq_args, integer()} | {min_args, integer()}.

-opaque command(MTT) :: {command,
        fun((named_args(), list(binary()), flags()) -> MTT),
        flags(),
        binary(),
        gleam@option:option(args_count()),
        list(binary())}.

-type internal_command(MTU) :: {internal_command,
        fun((named_args(), list(binary()), flags()) -> MTU),
        flags(),
        gleam@option:option(args_count()),
        list(binary())}.
...
-type command_node(MTV) :: {command_node,
        gleam@option:option(internal_command(MTV)),
        gleam@dict:dict(binary(), command_node(MTV)),
        flags(),
        binary()}.

-type out(MTW) :: {out, MTW} | {help, binary()}.
...
-opaque flag(MTX) :: {flag,
        binary(),
        binary(),
        fun((binary()) -> {ok, MTX} | {error, snag:snag()}),
        fun((flag_internals(MTX)) -> value()),
        fun((flags(), binary()) -> {ok, MTX} | {error, snag:snag()}),
        gleam@option:option(MTX)}.

-type flag_internals(MTY) :: {flag_internals,
        gleam@option:option(MTY),
        fun((binary()) -> {ok, MTY} | {error, snag:snag()})}.
...
-spec pretty_help(glint(MUF), pretty_help()) -> glint(MUF).
...
-spec with_name(glint(MUI), binary()) -> glint(MUI).
...
-spec without_exit(glint(MUL)) -> glint(MUL).
without_exit(Glint) ->
    erlang:setelement(
        2,
        Glint,
...
-spec as_module(glint(MUO)) -> glint(MUO).
as_module(Glint) ->
    erlang:setelement(
        2,
        Glint,
...
-spec with_indent_width(glint(MUR), integer()) -> glint(MUR).
...
-spec with_max_output_width(glint(MUU), integer()) -> glint(MUU).
...
-spec with_min_first_column_width(glint(MUX), integer()) -> glint(MUX).
...
-spec with_column_gap(glint(MVA), integer()) -> glint(MVA).
...
-spec global_help(glint(MVJ), binary()) -> glint(MVJ).
...
-spec command_help(binary(), fun(() -> command(MVY))) -> command(MVY).
command_help(Desc, F) ->
    erlang:setelement(4, F(), Desc).

-file("/home/runner/work/glint/glint/src/glint.gleam", 345).
-spec unnamed_args(args_count(), fun(() -> command(MWB))) -> command(MWB).
...
-spec named_arg(
    binary(),
    fun((fun((named_args()) -> binary())) -> command(MWE))
) -> command(MWE).
named_arg(Name, F) ->
    Cmd = (F(
        fun(Named_args) ->
...
-spec new_builder(
    binary(),
    fun((flag_internals(MYS)) -> value()),
    fun((flags(), binary()) -> {ok, MYS} | {error, snag:snag()}),
    fun((binary()) -> {ok, MYS} | {error, snag:snag()})
) -> flag(MYS).
...
-spec wrap_with_constraint(
    fun((binary()) -> {ok, MZE} | {error, snag:snag()}),
    fun((MZE) -> {ok, MZE} | {error, snag:snag()})
) -> fun((binary()) -> {ok, MZE} | {error, snag:snag()}).
...
-spec flag_constraint(flag(MZA), fun((MZA) -> {ok, MZA} | {error, snag:snag()})) -> flag(MZA).
...
-spec flag(
    flag(MWH),
    fun((fun((flags()) -> {ok, MWH} | {error, snag:snag()})) -> command(MWK))
) -> command(MWK).
flag(Flag, F) ->
    Cmd = F(
        fun(_capture) ->
...
-spec fold(flags(), MZZ, fun((MZZ, binary(), flag_entry()) -> MZZ)) -> MZZ.
fold(Flags, Acc, F) ->
...
-spec command(fun((named_args(), list(binary()), flags()) -> MVV)) -> command(MVV).
command(Runner) ->
...
-spec get_value(
    flags(),
    binary(),
    fun((flag_entry()) -> {ok, NAM} | {error, snag:snag()})
) -> {ok, NAM} | {error, snag:snag()}.
...
-spec get_flag(flags(), flag(NAP)) -> {ok, NAP} | {error, snag:snag()}.
get_flag(Flags, Flag) ->
...
-spec do_update_at(
    command_node(NBI),
    list(binary()),
    fun((command_node(NBI)) -> command_node(NBI))
) -> command_node(NBI).
...
-spec update_at(
    glint(NBC),
    list(binary()),
    fun((command_node(NBC)) -> command_node(NBC))
) -> glint(NBC).
...
-spec path_help(glint(MVF), list(binary()), binary()) -> glint(MVF).
...
-spec add(glint(MVM), list(binary()), command(MVM)) -> glint(MVM).
...
-spec group_flag(glint(MWN), list(binary()), flag(any())) -> glint(MWN).
...
-spec execute(glint(MWT), list(binary())) -> {ok, out(MWT)} | {error, binary()}.
execute(Glint, Args) ->
    Help_flag = <<"--"/utf8,
        (erlang:element(
            2,
...
-spec run_and_handle(glint(MXS), list(binary()), fun((MXS) -> any())) -> nil.
