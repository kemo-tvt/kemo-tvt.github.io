...
-spec one_of(list(MMQ)) -> fun((MMQ) -> {ok, MMQ} | {error, snag:snag()}).
one_of(Allowed) ->
...
-spec none_of(list(MMT)) -> fun((MMT) -> {ok, MMT} | {error, snag:snag()}).
none_of(Disallowed) ->
...
-spec each(fun((MMW) -> {ok, MMW} | {error, snag:snag()})) -> fun((list(MMW)) -> {ok,
        list(MMW)} |
    {error, snag:snag()}).
each(Constraint) ->
