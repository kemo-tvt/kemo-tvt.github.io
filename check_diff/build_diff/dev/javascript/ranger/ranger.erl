...
-spec create(
    ...
    fun((FWN) -> FWN),
    fun((FWM, FWN) -> FWM),
    fun((FWM, FWM) -> gleam@order:order())
) -> fun((FWM, FWM, FWN) -> {ok, gleam@iterator:iterator(FWM)} | {error, nil}).
...
-spec create_infinite(
    fun((FWR) -> boolean()),
    fun((FWR, FWS) -> FWR),
    fun((FWR, FWR) -> gleam@order:order())
) -> fun((FWR, FWS) -> {ok, gleam@iterator:iterator(FWR)} | {error, nil}).
